...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  const pageTitle = locale === 'de' ? 'Impressum' : 'Imprint'

  return {
    title: siteTitle ? `${pageTitle} | ${siteTitle}` : pageTitle,
    alternates: {
      canonical: `/impressum?lang=${locale}`,
      languages: {
        de: '/impressum?lang=de',
...
function getLocalizedValue(value: unknown, locale: Locale, fallback: string = ''): string {
...
function extractLexicalText(root: unknown): string {
  const walk = (node: unknown): string => {
    ...
    if (
      ...
      return (node as { children: unknown[] }).children.map(walk).join('')
    }
    return ''
  }
  return walk(root)
...
export default async function ImpressumPage({ searchParams }: Props) {
  ...
  try {
    ...
    impressumData = impressumResult.docs?.[0] ?? null
  } catch (error) {
  ...
  try {
    if (
      ...
      typeof impressumData.content === 'object' &&
      impressumData.content !== null
    ) {
      if ('root' in impressumData.content) {
    ...
    if (
      ...
      typeof impressumData.content === 'object' &&
      impressumData.content !== null &&
      'root' in impressumData.content
    ) {
      const extracted = extractLexicalText((impressumData.content as { root: unknown }).root)

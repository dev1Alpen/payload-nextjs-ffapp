...
export default async function DatenschutzPage({ searchParams }: Props) {
  ...
  // Fetch datenschutz content from CMS
  ...
  try {
    ...
    // Fetch datenschutz content (should only be one record)
    try {
  ...
  const description = datenschutzData
    ? getLocalizedValue(datenschutzData.description, locale, fallbackTranslations[locale].description)
  ...
  try {
    ...
    if (datenschutzData?.content && typeof datenschutzData.content === 'object' && 'root' in datenschutzData.content) {

...
export default async function ReservePage({ searchParams }: Props) {
  ...
  // Fetch reserve page data from CMS
  const pageData = {
    title: locale === 'de' ? 'Reserve der FF-Droß' : 'Reserve of FF-Droß',
    intro: locale === 'de' 
  ...
  try {
    ...
    // Fetch the first reserve document (typically there will be only one)
    const reserveResult = await payload.find({
      collection: 'reserve',
      locale,
      limit: 1,
    ...
    if (reserveResult.docs.length > 0) {
      const reserve = reserveResult.docs[0]
      pageData.title = reserve.title || pageData.title
      pageData.intro = reserve.intro || pageData.intro
      
      // Process team members
      if (reserve.teamMembers && Array.isArray(reserve.teamMembers)) {
        // Sort by order
        const sortedMembers = [...reserve.teamMembers].sort((a: any, b: any) => {
          const orderA = a.order ?? 0
          const orderB = b.order ?? 0
          return orderA - orderB
        })
        
        reserveTeam = sortedMembers.map((member: any, index: number) => {
    ...
    console.error('Error fetching reserve page data:', error)
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 items-start">
                {reserveTeam.map((member, index) => (
                  <TeamMemberCard
                    ...
                    locale={locale}
                    index={index}
                    gradientIdPrefix="reserveBorderGrad"
                  />
                ))}
              </div>
            </div>

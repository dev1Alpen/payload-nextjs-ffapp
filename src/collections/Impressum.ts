...
export const Impressum: CollectionConfig = {
  ...
  admin: {
    ...
    group: 'Pages',
    description: {
      en: 'Manage Impressum (Legal Notice) page content. Only one record can exist.',
      de: 'Verwalten Sie den Inhalt der Impressum-Seite. Es kann nur ein Datensatz existieren.',
    },
  },
  access: {
    read: ({ req: { user } }) => {
      // Public can read published impressum
      if (!user) {
        return {
      ...
      async ({ data, req, operation }) => {
        ...
        try {
          ...
          if (existing.totalDocs > 0) {
            throw new Error('Only one Impressum record is allowed. Please update the existing record instead.')
          }
        } catch (error) {
          // If it's our custom error, re-throw it
          if (error instanceof Error && error.message.includes('Only one Impressum record')) {
            throw error
          }
          // Otherwise, log and continue (don't block creation if query fails)
          console.error('Error checking for existing Impressum:', error)
        }
